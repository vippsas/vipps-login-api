...
| address     | Users addresses given as claims 'address' and 'other_address'. The claim 'address' returns the address set as 'default' for the Vipps user. And the claim 'other_address' returns all other addresses of the end user, if any. One of the addresses will always be home, but can also include work and other, and would not necessary be marked as default. We recommend that merchants fetch all addresses on a user and allow the user to choose which address to use in the relevant context |   yes   |
