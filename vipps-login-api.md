...
| redirect_uri      | Redirect URL which the user agent is redirected to after finishing a login. If using a custom URL scheme, such as `myapp://` a path is required: `myapp://path-to-something`. See [API endpoints required by Vipps from the merchant](#api-endpoints-required-from-the-merchant)          |
