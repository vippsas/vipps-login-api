...
| address     | List containing the users addresses. Will always contain home, but can also include work and other.   The claim 'address' returns the address set as "default" for the Vipps user. The other addresses, if any, of the user are served under claim 'other_address' as a list. We recommend that merchants fetch all addresses on a user and allow the user to choose which address to use in the relevant context |   yes   |
