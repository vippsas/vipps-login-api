...
1. Send request `Get OIDC well-known` which uses [/access-management-1.0/access/.well-known/openid-configuration](https://vippsas.github.io/vipps-developer-docs/api/login#tag/Vipps-Login-API/operation/discoverOpenIDConfiguration).
...
1. From the `Vipps Login in Browser` folder, send request `Get token` which uses [access-management-1.0/access/oauth2/token][access-token-endpoint]. This provides the access token and id token.
1. Send request `Get user info` which uses uses [vipps-userinfo-api/userinfo][userinfo-endpoint] to get the user info of the logged-in user.
...
4. Send request `Token no-redirect` which uses which uses [access-management-1.0/access/oauth2/token][access-token-endpoint]. This will populate the environment variable `access_token` used for the subsequent request.
5. Send request to `Get user info` which uses uses [vipps-userinfo-api/userinfo][userinfo-endpoint]. This will use the token from (4) to obtain the userinfo of the logged-in user.
...
1. Submit the  `Token redirect` request which uses [access-management-1.0/access/oauth2/token][access-token-endpoint]. This will populate the environment variable `access_token` used for the subsequent request.
1. Send request to `Get user info` which uses uses [vipps-userinfo-api/userinfo][userinfo-endpoint]. This will use the token from the previous step to obtain the user info of the logged-in user.
